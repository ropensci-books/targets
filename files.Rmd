...
1. Finds all the `tar_load()`/`tar_read()` dependencies in the report and inserts them into the target's command. This enforces the proper dependency relationships.
...
1. Sets convenient default options such as `deployment = "local"` in `tar_target()` and `quiet = TRUE` in `rmarkdown::render()`.
...
Because symbols `fit` and `hist` appear in the command, `targets` knows that `report` depends on `fit` and `hist`. These dependency relationships show up in the graph.
