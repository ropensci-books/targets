...
An R Markdown report should be lightweight: mostly prose, minimal code, fast execution, and no output other than the rendered HTML/PDF document. Here, R Markdown reports are specialized targets to document the work the pipeline already did. The code chunks are just calls to  `tar_read()` and `tar_load()`.
