...
Thus, different targets should have non-identical sequences of pseudo-random numbers.^[However, there is a small chance that different random number generator sequences may overlap, even if they have different seeds. It is unclear how much of a risk this is in practice. For details, read the "Random number generation" section of the `parallel` package vignette: `vignette("parallel", package = "parallel")`. The authors propose a safeguard using widely-spaced L'Ecuyer streams. However, because `parallel::nextRNGStream()` generates each stream recursively, their solution is unfortunately not feasible for `targets`. There is an issue thread at <https://github.com/ropensci/targets/issues/1139>, and extra safeguards against overlap may be adopted if they turn out to be possible. Also see [L'Ecuyer et al. (2017)](https://www.sciencedirect.com/science/article/pii/S0378475416300829).]
