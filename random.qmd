...
This chapter was written for `targets` version `1.3.2.9001` and higher, which implements `tar_seed_create()` and additional safeguards for stastical independence of pseudo-random numbers.
:::
...
In theory, these parallel random number generator streams could overlap and produce statistically correlated results. However, the risk is extremely small in practice. See the "RNG overlap" section of <https://docs.ropensci.org/targets/reference/tar_seed_create.html> for details, references, and justification.
