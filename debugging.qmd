...
Sometimes, however, you may not see the original error:
...
Above, the random number generator seed in your local session is different from the seed assigned to the target in the pipeline. The dataset from the pipeline has missing values, whereas the one in the local session does not. If you cannot reproduce the error in an interactive R session, read on.
...
In addition, try running your code with just `callr` and not `targets`:
...
If you successfully reproduce the bug without using `targets`, then the problem becomes much smaller and much easier to solve, and you can completely exclude `targets` from the rest of the debugging process.
