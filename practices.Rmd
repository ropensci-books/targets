...
While the `imports` option tells `targets` what to scan, it does not actually load any packages. So in practice, please set the `packages` option as well, e.g. `tar_option_set(imports = "package1", packages = c("package1", "tidyverse"))`. `imports` and `packages` operate independently so that you have complete control over which packages load first (in case of conflicts).^[Complicated package conflicts may require more work to untangle. If you use [conflicted](https://github.com/r-lib/conflicted), please create a project-level `.Rprofile` file and call [conflict_prefer()](https://conflicted.r-lib.org/reference/conflict_prefer.html) from there. `.Rprofile` is safer than `_targets.R` for this because it ensures the appropriate global options get set on remote workers as well as the main process.]
