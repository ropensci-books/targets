...
Remember: with `tar_make_clustermq()`, the workers are *persistent*. A persistent worker runs multiple targets, and it is not possible to predict which targets will be assigned to any given persistent worker. So in the `resources` argument of `tar_target()`, any `clustermq`-specific will be ignored. `tar_option_set()` is the only way to set `clustermq` resources.
:::
