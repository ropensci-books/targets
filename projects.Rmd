...
This approach is the most convenient, but it can be inefficient if `target_from_project_a` is large. A higher-performant solution for large data is to treat the file in project A's data store as an [input file target](#data) in project B. (Unfortunately, this does break the conceptual abstraction of the `targets` data store.)
