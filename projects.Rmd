...
All projects share the same functions defined in the scripts in `R/`, and each project uses a different [target script](https://docs.ropensci.org/targets/reference/tar_script.html) and [data store](#files). `script_a.R` defines the targets for project A.
...
Alternatively, you can manually select the appropriate script and store for each project. This is a less convenient approach, but you do it, you do not need to set the `TAR_PROJECT` environment variable or rely on `_targets.yaml`.
...
`_targets.yaml` can control more than just the script and store, and different project can inherit settings from one another. In the following example, project B inherits from project A, so projects A and B both set `reporter = "summary"` and `shorcut = TRUE` by default in `tar_make()`.
...
This approach is the most convenient, but it can be inefficient if `target_from_project_a` is large. A higher-performant solution for large data is to treat the file in project A's data store as an [input file target](#files) in project B. (Unfortunately, this does break the conceptual abstraction of the `targets` data store.)
