...
This approach is the most convenient, but it can be inefficient if `target_from_project_a` is large. A higher-performant solution for large data is to export the target from Project A as an external data file and then establish that file as a file target in Project B.
...
list(
  ...,
  tar_target(results_a, final_results(previous_target)),
  tar_target(file_a, {
    saveRDS(results_a, "results_a.rds")
    "results_a.rds"
  })
)
```
...
tar_option_set(packages = "tidyverse")
list(
  tar_target(file_from_project_a, "results_a.rds", format = "file"),
  tar_target(data_from_project_a, readRDS(file_from_project_a)),
