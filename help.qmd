...
Your issue could have any number of causes. It could be a bug in `targets`, a misunderstanding of how to use `targets`, a bug in a different R package, a mistake in one of your R functions, or hundreds of other reasons. In addition, you may be the first user to experience the problem. So when you ask for help, it is not enough to state the error and ask what the cause might be. There are too many possibilities, unknowns, and hidden assumptions to answer to a vague or incomplete question about code. Not even an expert in R or `targets` could guess, no matter how much prior knowledge or experience they have.
