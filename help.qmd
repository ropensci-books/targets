...
The cause of your issue could be a bug in `targets`, a misuse of `targets`, a bug in a different R package, a mistake in one of your R functions, or some other mysterious or unexpected reason. There are hundreds of strange possibilities. In addition, you may be the first user to experience the problem. So when you ask for help, it is not enough to state the error and ask what might be the cause. There are too many unknowns and hidden assumptions to answer a vague or incomplete question about code. The true answer is usually surprising, even for experts. Not even a master of R or `targets` could guess, no matter how much prior knowledge or experience they have.

To receive constructive help, please provide the complete context of your use case, including a reprex. A reprex, or reproducible example^[Also known as a minimal reproducible example or minimal working example.], is a piece of complete, self-contained, readable, runnable code that fully emulates the problem. The code should reproduce the issue, [look clean](https://style.tidyverse.org/), have as few lines as possible, run in as few seconds as possible, and contain only the details most relevant to the problem. You can embed the code inline in your question, or you can upload it to a public repository and post the link. Regardless, you should expect that anyone trying to help will read all the code and run the reprex's `_targets.R` file on their own computer without access to any of the files on your own private machine. Please make this empirical process as quick and easy as possible for people who volunteer their valuable time and energy to respond to you.
