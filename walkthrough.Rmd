...
`functions.R` contains our custom user-defined functions. (See the [functions chapter](#functions) for a discussion of function-oriented workflows.)
...
Before you run the pipeline for real, it is best to check for errors. [`tar_manifest()`](https://docs.ropensci.org/targets/reference/tar_manifest.html) lists verbose information about each target.

```{r, eval = TRUE}
tar_manifest(fields = "command")
```

[`tar_glimpse()`](https://docs.ropensci.org/targets/reference/tar_glimpse.html) and [`tar_visnetwork()`](https://docs.ropensci.org/targets/reference/tar_visnetwork.html) visualize the pipeline as a directed acyclic graph (DAG).^[If [`tar_manifest()`](https://docs.ropensci.org/targets/reference/tar_manifest.html) shows hundreds of targets, then the graph may be slow or cumbersome. If this happens, then you may need to comment out some targets in the `_targets.R` file just for visualization purposes.]

```{r, eval = TRUE}
tar_visnetwork()
```

The directed edges in the graph tell you how the targets and functions depend on one another. For example, if target `x` is supposed to call `get_data()` but there is no directed edge from `x` to `get_data()`, then you may have misspelled `get_data()` in the command of `x`. Most pipelines have a completely connected graph with a clear flow of work from left to right. If your graph looks clean, you may be ready to run the pipeline.^[The DAG is indifferent to the order of targets in your pipeline. You will still get the same graph even if you rearrange them. This is because `targets` uses static code analysis to detect the dependencies of each target, and this process does not depend on target order. For details, visit the dependency detection section of the [target construction chapter](#targets).]
