...
1. A literate programming source document (or Quarto project) that renders inside an individual target. Here, you define a special kind of target that runs a lightweight R Markdown report or Quarto report/project which depends on upstream targets.
2. The [`tar_tangle()`](https://docs.ropensci.org/tarchetypes/reference/tar_tangle.html#details) function available in `tarchetypes` >= 0.13.2.9002, which lets you express the pipeline itself as a Quarto or R Markdown document. 
