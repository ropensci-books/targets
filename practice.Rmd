...
Adept pipeline construction requires an understanding of dependency detection. To identify the targets and global objects that each target depends on, the `targets` package uses static code analysis with `codetools::findGlobals()`. Let us look at the dependencies of the `raw_data` target.
...
Not all of the objects from `codetools::findGlobals()` actually register as dependencies. When it comes to detecting dependencies, `targets` only recognizes
