...
If you set `error = "save"` in `tar_option_set()` or `tar_target()`, a target will save its workspace to a compressed file in `_targets/workspaces/` if it errors out. Objects in the workspace include global objects (e.g. functions), dependency targets, the random number generator seed, and the traceback. You can load this workspace and set the target's seed with `tar_workspace()`. Workspace files can be quite large, and you can remove them all with `tar_undebug()`.
