...
If you set `error = "save"` in `tar_option_set()` or `tar_target()`, a target will save its workspace to a compressed file in `_targets/workspaces/` if it errors out. Objects in the workspace include a listing of the target's required packages, the global objects (e.g. functions), dependency targets, the random number generator seed, and the traceback. You can load this workspace with `tar_workspace()`. `tar_workspace()` loads the packages, populates the objects in the environment, and sets the seed to the seed of the target. Workspace files can be quite large, so you should remove them all with `tar_undebug()` when you are done debugging.
