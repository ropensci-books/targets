...
In addition to configuration options hard-coded in the template file, you can supply computing resources to individual targets with the `template` argument of `tar_options()`. For example, if your template has a wildcard `num_cores` for the number of cores per worker,
...
The `tar_target()`, `tar_target_external()`, and `tar_options()` functions accept a `resources` argument similar to the `template` argument of `tar_make_clustermq()`. For example, if our `batchtools` template file has a wildcard for the number of cores for a job,
...
Functions `tar_target()`, `tar_target_external()`, and `tar_options()` support advanced configuration options for heavy-duty pipelines that require high-performance computing.
...
* `memory`: Choose whether to retain a target in memory or remove it from memory whenever it is not needed at the moment. This is a tradeoff between memory consumption and storage read speeds, and like all of the options listed here, you can set it on a target-by-target basis. The default settings consume a lot of memory to avoid frequently reading from storage. To keep memory usage down to a minimum, set `memory = "transient"` in `tar_options()` and set `garbage_collection = TRUE` in your `tar_make*()` function.
