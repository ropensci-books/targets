...
The `tar_target()`, `tar_target_raw()`, and `tar_options()` functions accept a `resources` argument similar to the `template` argument of `tar_make_clustermq()`. For example, if our `batchtools` template file has a wildcard for the number of cores for a job,
...
Functions `tar_target()`, `tar_target_raw()`, and `tar_options()` support advanced configuration options for heavy-duty pipelines that require high-performance computing.
