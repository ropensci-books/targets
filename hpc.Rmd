...
The `use_targets()` function configures `future` and/or `future.batchtools` automatically (if those packages are installed). It tries to detect the cluster you are using, if any, and write the appropriate `_targets.R` settings `clustermq` configuration files commensurate with your system resources. On most systems, you should be able to run e.g. `tar_make_future(workers = 2)` right away. In case you have specific configuration needs, however, read on to learn how to set it up yourself.
