# Integration with Amazon S3 storage {#storage_amazon}
...
As of version 0.8.1.9000, `targets` uses the [`paws`](https://github.com/paws-r/paws) package behind the scenes. It is not a strict dependency of `targets`, so you will need to install it yourself.

```{r, eval = FALSE}
install.packages("paws")
```

Next, `paws` needs an access ID, secret access key, and default region. Please refer to the credentials section of the [`paws` README](https://github.com/paws-r/paws/blob/main/README.md) for a list of the appropriate environment variables. Then, set these values in your user-level `.Renviron` with `usethis::edit_r_environ()`. Visit <https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-envvars.html> for more details. Here is an example `.Renviron` file. 
...
Optionally, enable versionining 

`targets` >= 0.9.0 tracks the version IDs of objects in versioning-enabled buckets. (A later section discusses data version control.) You can enable versioning in your bucket through `paws` or the AWS web console.
...
#> list()
s3$get_bucket_versioning("my-test-bucket-25edb4956460647d")$Status
#> [1] "Enabled"
```

Sign in to <https://s3.console.aws.amazon.com/s3> to verify that the bucket exists and has the desired versioning configuration.
...
Lastly, if you want to erase the whole project or start over from scratch, consider removing the S3 bucket to avoid incurring storage fees. The easiest way to do this is through the [S3 console](https://s3.console.aws.amazon.com/s3). Alternatively, you can programmatically delete the bucket using functions in `paws`. Either way, the bucket must be empty before it is deleted, including past versions of objects in a versioning-enabled bucket.

## Data version control

As of version 0.9.0, `targets` supports built-in native version control through AWS S3 storage. If your bucket has [versioning enabled](https://docs.aws.amazon.com/AmazonS3/latest/userguide/Versioning.html), then `targets` records the version ID of each target (in `tar_meta(your_target)$path`), and it uses that version ID to read the data and check the status. So if you revert the metadata file `_targets/meta/meta` to a prior state, then your pipeline will automatically revert to prior versions of your targets. This allows you to quickly switch your entire project to a [previous state](https://git-scm.com/docs/git-checkout) or [alternate branch](https://git-scm.com/docs/git-branch) while keeping your AWS-backed targets in sync and up to date. It works best if you commit  `_targets/meta/meta` to the same [version control repository](https://git-scm.com) you use for your code. (<https://happygitwithr.com> is an excellent resource to get started with version control for code.)

## Alternative endpoints

As of version 0.11.0, `targets` supports an `endpoint` argument of `tar_resources_aws()`. For example, `tar_resources_aws(endpoint = "https://storage.googleapis.com", region = "auto")` connects the AWS functionality to Google Cloud Storage (GCS). However, as in the case of GCS, versioning support may no longer work. To check, run with a versioned bucket and check that the version ID is part of `tar_meta(your_target_name, path)$path`. For version-capable GCS support, please read the [Google Cloud Storage chapter](#storage_google).
