...
To check the correctness of a pattern without running the pipeline, use [`tar_pattern()`](https://wlandau.github.io/targets/reference/tar_pattern.html). First, make up some branch/bud names for the upstream dependencies. They need not be realistic, but they should be characters.

```{r}
w_comp <- c("w_comp_550d7456", "w_comp_a20cadbf")
x_comp <- c("x_comp_42f35290", "x_comp_f95ad1c7", "x_comp_e2eccaa5")
y_comp <- c("y_comp_67188e74", "y_comp_5512ec97", "y_comp_4b1cfc1b")
```

Next, supply the pattern and names to [`tar_pattern()`](https://wlandau.github.io/targets/reference/tar_pattern.html).
