...
You can compose multiple patterns to more finely control the branches you create. Below, target `z` creates six branches, one for each combination of `w` and tuple (`x`, `y`). The pattern `cross(w, map(x, y))` is equivalent to `tidyr::crossing(w, tidyr::nesting(x, y))`.
