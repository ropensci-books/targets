...
And here is a demonstration of dynamic slicing.
...
tar_script({
  ...
  list(
    ...
    tar_target(
      a_sliced_data,
      a_data,
      pattern = slice(a_data, index = seq(5, 15))
    ),
    tar_target(
      a_some_branches,
      toupper(a_sliced_data),
      pattern = slice(a_sliced_data, index = c(2, 3))
    )
  )
})
```
...
```{r, eval = TRUE}
# R console
tar_read(a_sliced_data)
```

```{r, eval = TRUE}
# R console
tar_read(a_some_branches)
```
