...
As expected the first target has the name `gapminder` and the expression `get_data("gapminder")`.

```{r}
data_target_list[[1]]
```

In practice, we use the above target list to build our pipeline in `_targets.R`.
