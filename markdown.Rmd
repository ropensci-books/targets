...
The `{targets}` engine can run in interactive or non-interactive mode. Interactive mode is for prototyping and testing in the [notebook interface](https://bookdown.org/yihui/rmarkdown/notebook.html), while non-interactive mode is for pipeline construction during `knitr::knit()` or `rmarkdown::render()`. The default mode is chosen automatically^[In `targets` version 0.6.0, the mode is interactive if `interactive()` is `TRUE`. In subsequent versions, the mode is interactive if `!isTRUE(getOption("knitr.in.progress"))` is `TRUE`.], and you can set it with the `tar_interactive` chunk option:
