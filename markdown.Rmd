...
In non-interactive mode, the chunk overwrites the `_targets.R` file and writes the R code to a script in `_targets_r/globals/`. It is good practice to assign chunk labels because they are used as script names. (The labels of unnamed chunks depend on chunk order, which is brittle).^[All chunk labels must be unique. And for `bookdown` projects, chunk labels should only use alphanumeric characters and dashes.]
...
For demonstration purposes, here is the same chunk in non-interactive mode. In non-interactive mode, the `{targets}` engine does not actually run any targets. Instead, it overwrites `_targets.R` and writes the code to a script in `_targets_r/targets/`.
...
In interactive mode, it is good practice to inspect the outputs to make sure the targets ran properly. Remember: in interactive mode, these results do not exist in permanent storage, and they will evaporate when you restart your R session.

```{targets downstream-targets-int, tar_interactive = TRUE, echo = FALSE, results = "hide"}
list(
  tar_target(data, raw_data %>% filter(!is.na(Ozone))),
  tar_target(hist, create_plot(data)),
  tar_target(fit, biglm(Ozone ~ Wind + Temp, data))
